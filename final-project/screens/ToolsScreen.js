...
export default function ToolsScreen() {
  ...
  var user = firebaseApp.auth().currentUser.uid.toString(); 
  ...
  async function addGameToDatabase(gameName, key, imageUrl, price) {
    if(user == "XjA4BoMlXWgQWwcvJgk1TcCvCL82") {
    ...
    if (valid) {
      ...
      alert("Only admins can modify database..")
    }
  }
...
//   alert("Must be logged in as admin to use tools.")
//   return window.location = "http://localhost:19006/root/Inventory";
return (
  < ScrollView style={styles.container} contentContainerStyle={styles.contentContainer} >
    <View style={styles.inputContainer}>
      <Text>Must be logged in...</Text>
    </View>
  </ScrollView >
);
 }
